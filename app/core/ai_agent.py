...
def _sanitize_xslt(xslt_str: str) -> str:
    """
    Normalize XSLT output across providers so the emitted structure is the same
    whether OpenAI or Bedrock generated it. gpt-4o-mini frequently appends an
    extra catch-all/identity template alongside the intended single template;
    Claude does not. This collapses output to the one meaningful template.
    """
    if not xslt_str:
        return xslt_str
    ...
    xslt_str = re.sub(r'^\s*' + fence + r'(?:xml|xslt)?\s*', '', xslt_str)
    xslt_str = re.sub(r'\s*' + fence + r'\s*$', '', xslt_str).strip()

    # 2) Remove self-closing catch-all templates: <xsl:template match="text()"/>
    xslt_str = re.sub(r'<xsl:template\b[^>]*/>\s*', '', xslt_str, flags=re.IGNORECASE)

    # 3) If more than one full template block remains, keep only the meaningful
    #    one (the block that emits fields), drop identity/catch-all blocks.
    block_re = re.compile(r'<xsl:template\b[^>]*>.*?</xsl:template>',
                          re.IGNORECASE | re.DOTALL)
    blocks = block_re.findall(xslt_str)

    if len(blocks) > 1:
        meaningful = [b for b in blocks
                      if '<xsl:choose' in b.lower()
                      or '<response' in b.lower()
                      or '<xsl:value-of' in b.lower()]
        keep = meaningful[0] if meaningful else blocks[0]
        first_start = xslt_str.lower().find('<xsl:template')
        last_end = xslt_str.lower().rfind('</xsl:template>') + len('</xsl:template>')
        xslt_str = xslt_str[:first_start] + keep + xslt_str[last_end:]

    return xslt_str.strip()
...
10. Output EXACTLY ONE <xsl:template> element. Do NOT add any identity transform or catch-all template (e.g. match="@*|node()" or match="text()"). One template only.
"""
    ...
    try:
        ...
        xslt_str = _sanitize_xslt(xslt_str)
