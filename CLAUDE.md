...
Six Oracle tables are populated sequentially from the touchpoint
detail page:
...
5. **MASHUPQUERYPARAMETER** — via Finish Configuration (request template
   + input params from ##KEY## placeholders)
6. **MASHUPPARAMMAPPING** — via Finish Configuration (parameter→InputParamN
   positional mapping, MAPPINGID auto-generated by Oracle trigger)
