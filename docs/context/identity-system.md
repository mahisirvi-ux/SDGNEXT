...
> **Note:** `IDRFunctional` is no longer a workflow gate. It serves as the
> metadata store for touchpoints. Default `idr_status` is now `"Signed-Off"`.
> Owner-like fields (`owner`, `module_owner_functional`, `technical_owner`,
> `pending_with`) continue to be validated via the identity validator on
> write and enriched on read. The Phase 1 UI has been removed; the Workshop
> Board reads these fields directly.
...
| `"System (Workshop Mailer)"` | `workshop_mailer.py:send_workshop_invites` | Generated when a workshop invite email is dispatched for a touchpoint. |
| `"System (MoM Nudge)"` | `email_engine.py:send_mom_pointer_nudges` | Generated by the daily MoM-pointer nudge job for MoM-spawned follow-ups. |
...
| `"MOM_SENT"` | `mom.py:send_session_mom_endpoint` | Logged atomically when a touchpoint MoM email is dispatched. Comment format: "MoM session #{id} ({date}) emailed to {N} recipients; {K} follow-ups spawned;MSG_ID=<message-id>". MSG_ID is used to anchor subsequent MoM-pointer nudges into the same email thread for that touchpoint. |
| `"MOM_NUDGE_SENT"` | `email_engine.py:send_mom_pointer_nudges` | Logged when a per-touchpoint MoM-pointer nudge email is dispatched. Comment includes count of open items and recipient count. Threaded onto earliest MOM_SENT for the same touchpoint via In-Reply-To/References. |
...
| `"WORKSHOP_INVITE_SENT"` | `workshop_mailer.py:send_workshop_invites` | Logged when a workshop invite email is dispatched. Comment contains structured data: `WORKSHOP_DATE=<iso>;KIND=fresh|rescheduled;RECIPIENTS_COUNT=<n>;MSG_ID=<message-id>`. MSG_ID is used to reconstruct the email thread (In-Reply-To/References headers) for subsequent rescheduled invites in the same (project, department) scope. Used for reschedule detection and duplicate prevention. |
...
| Workshop invite missing for owner | Owner name on `IDRFunctional.owner` not in `team_master` OR no department email configured |
